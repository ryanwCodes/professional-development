...
function filterDict(dict, callback) {
    const dictKeys = Object.keys(dict);
    const newDict = {};
    for (let i = 0; i < dictKeys.length; i++) {
        const currentKey = dictKeys[i];
...
function reduceDict(dict, callback, defaultAcc) {
    const dictKeys = Object.keys(dict);
    let acc = defaultAcc;
    for (let i = 0; i < dictKeys.length; i++) {
        const currentKey = dictKeys[i];
...
const oneOfEachFruitMass = reduceDict(fruits, (currentMass, fruit) => currentMass + fruit.mass, 
